...
namespace GameEntities
{
	public class WaterPlaneType : MapGeneralObjectType
	{
		...
		public class SplashItem
		{
			...
			[Editor( "GameEntities.Editor.WaterPlaneTypeSplashItem_ParticlesCollectionEditor, GameEntities.Editor", typeof( UITypeEditor ) )]
		...
		[TypeConverter( typeof( CollectionTypeConverter ) )]
		[Editor( "GameEntities.Editor.WaterPlaneType_SplashesCollectionEditor, GameEntities.Editor", typeof( UITypeEditor ) )]
	...
	public class WaterPlaneHighLevelMaterial : HighLevelMaterial
	{
		...
		void CreateDefaultTechnique()
		{
			...
			if( RenderSystem.Instance.HasShaderModel3() )
				compileArguments.Append( " -DSHADER_MODEL_3" );
			if( !transparent )
	...
	public class WaterPlane : MapGeneralObject
	{
		...
		void UpdateReflectionTexture( ReflectionTextureItem item )
		{
			...
			{
				if( volume.IsEditorExcludedFromWorld() )
					continue;
		...
		protected override void OnRender( Camera camera )
		{
			...
			if( EntitySystemWorld.Instance.IsEditor() && !camera.IsForShadowMapGeneration() &&
				MapEditorInterface.Instance.GetSelectedEntities().Contains( this ) )
			{
				//render volume
