...
namespace GameEntities
{
	...
	public class CharacterType : UnitType
	{
		...
		const float bottomRadiusDefault = .1f;
		[FieldSerialize]
		...
		const float walkForceDefault = 4000;
		[FieldSerialize]
		...
		const float runForceDefault = 6000;
		[FieldSerialize]
		...
		const float flyControlForceDefault = 500;
		[FieldSerialize]
	...
	public class Character : Unit
	{
		...
		{
			return mainBodyGroundDistance - .1f < Type.FloorHeight && groundBody != null;
		}
		...
		protected override void OnPostCreate( bool loaded )
		{
			...
			if( PhysicsWorld.Instance.IsPhysX() )
			{
				...
				//create down box
				{
					...
					shape.Dimensions = new Vec3( Type.BottomRadius * 2, Type.BottomRadius * 2,
						Type.Height - Type.BottomRadius * 2 );
				...
				{
					float radiusScale = 1.3f;

					SphereShape shape = body.CreateSphereShape();
					shape.Radius = Type.BottomRadius * radiusScale;
					shape.Position = new Vec3( 0, 0,
						( Type.Height - Type.WalkUpHeight ) / 2 - Type.Height + Type.BottomRadius * radiusScale );
					shape.ContactGroup = (int)ContactGroup.Dynamic;
					shape.StaticFriction = 0;
					shape.DynamicFriction = 0;
					shape.Bounciness = 0;
